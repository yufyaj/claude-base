# Git
- コミットはアトミック（最小で意味のある単位）に保つ
- 1コミット = 1つの論理的変更
- フォーマット変更とロジック変更は別コミットにする
- 各コミットはテストが通る状態を維持する

# Web検索
- 検索クエリに年を含める場合、必ず事前にコマンドで現在の日付を取得してから正確な年を使用する

# ライブラリ選定
- ライブラリを選定する際は、必ずWebで最新の情報を調べてから選定する
- メンテナンス状況、人気度、最終更新日を確認する

# エラーハンドリング
- try-catchのスコープは最小限に保つ（例外が発生しうる箇所のみを囲む）
- 広範囲を囲まず、エラーの発生源を特定しやすくする

# コード設計（SOLID原則に従う）
- 1つのクラス・関数には1つの役割だけを持たせる
- 新機能は既存コードを修正せず、新しいクラスや関数を追加して実装する
- 継承より合成（コンポジション）を優先する
- 不要なメソッドを持つインターフェースを実装させない
- 具体的なクラスではなく、インターフェースや抽象クラスに依存する
- 重複コードは共通処理として抽出する
- マジックナンバーや固定文字列はハードコーディングせず、定数や設定ファイルに切り出す
- APIキー、パスワード等の機密情報はコードに含めず、環境変数を使用する

# 可読性
- 変数名・関数名は処理内容が分かる命名にする
- 複雑なロジックにはコメントで意図を説明する

# 実装方針
- 実装前に既存コードのパターンを確認し、一貫性を保つ
- 不明点は実装前に確認する
- 複雑な機能は一度に実装せず、小さなステップに分けて実装・テストする

# 型安全性
- any型の使用は避け、適切な型定義を行う

# セキュリティ（OWASP準拠）
- 入力バリデーション: サーバーサイドで必ず検証、許可リスト方式を優先
- 出力エンコーディング: XSS防止のため、出力時に適切にエスケープ
- 認証: パスワードはハッシュ化して保存、多要素認証を検討
- アクセス制御: 最小権限の原則、ロールベースで制御
- 暗号化: 機密データは保存時・通信時ともに暗号化、独自実装せず標準ライブラリを使用
- エラーハンドリング: 詳細なエラー情報をユーザーに露出しない、ログには記録
- SQLインジェクション対策: プリペアドステートメント/パラメータ化クエリを使用

# デバッグ
- console.logやprint等のデバッグコードは本番コードに残さない

# テスト駆動開発（TDD）
**鉄則: 失敗するテストなしに本番コードを書かない**

Red-Green-Refactorサイクル:
1. RED: 失敗するテストを1つ書く
2. 確認: テスト実行、期待通りに失敗（必須）
3. GREEN: テストを通す最小限のコード
4. 確認: テスト実行、通ることを確認（必須）
5. REFACTOR: テストが通る状態を維持しながら改善

詳細:
- [TDD基本](../docs/tdd/README.md)
- [アンチパターン](../docs/tdd/anti-patterns.md)
- [よくある言い訳と反論](../docs/tdd/excuses.md)
